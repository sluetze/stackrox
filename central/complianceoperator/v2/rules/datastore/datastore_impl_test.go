...
func (s *complianceRuleDataStoreTestSuite) TestUpsertRules() {
	// make sure we have nothing
	ruleIDs, err := s.storage.GetIDs(s.hasReadCtx)
	s.Require().NoError(err)
	s.Require().Empty(ruleIDs)
	...
	for _, tc := range testCases {
		if len(tc.rules) == tc.expectedRecordIndex.Cardinality() {
			s.Require().NoError(s.dataStore.UpsertRules(tc.testContext, tc.rules))
		} else {
			s.Require().Error(s.dataStore.UpsertRules(tc.testContext, tc.rules), "access to resource denied")
		}
...
func (s *complianceRuleDataStoreTestSuite) TestDeleteRule() {
	...
	for _, tc := range testCases {
		s.Require().NoError(s.dataStore.UpsertRules(s.hasWriteCtx, tc.rules))
