...
func (s *fullStoreImpl) readRows(
	...
	if len(plops) > 0 {
		...
		memoryDeploymentId := len([]byte(plops[0].DeploymentId)) * len(plops)
		memoryPodId := len([]byte(plops[0].PodId)) * len(plops)
		memoryPodUid := len([]byte(plops[0].PodUid)) * len(plops)
		memoryContainerName := len([]byte(plops[0].ContainerName)) * len(plops)
		memoryClusterId := len([]byte(plops[0].ClusterId)) * len(plops)
		memoryNamespace := len([]byte(plops[0].Namespace)) * len(plops)
		memoryImageId := len([]byte(plops[0].ImageId)) * len(plops)
		memorySignalId := len([]byte(plops[0].Signal.Id)) * len(plops)
		memorySignalContainerId := len([]byte(plops[0].Signal.ContainerId)) * len(plops)
		memorySignalName := len([]byte(plops[0].Signal.Name)) * len(plops)
		memorySignalArgs := len([]byte(plops[0].Signal.Args)) * len(plops)
		memorySignalExecFilePath := len([]byte(plops[0].Signal.ExecFilePath)) * len(plops)
		memoryTotal := memorySize + memorySizeValue + memorySizeSignal + memorySizeEndpoint
		memoryTotal += memoryDeploymentId + memoryPodId + memoryPodUid + memoryContainerName
		memoryTotal += memoryClusterId + memoryNamespace + memoryImageId
		memoryTotal += memorySignalId + memorySignalContainerId + memorySignalName
		memoryTotal += memorySignalArgs + memorySignalExecFilePath
		...
		log.Infof("plops memoryDeploymentId= %v", memoryDeploymentId)
		log.Infof("plops memoryPodId= %v", memoryPodId)
		log.Infof("plops memoryPodUid= %v", memoryPodUid)
		log.Infof("plops memoryContainerName= %v", memoryContainerName)
		log.Infof("plops memoryClusterId= %v", memoryClusterId)
		log.Infof("plops memoryNamespace= %v", memoryNamespace)
		log.Infof("plops memoryImageId= %v", memoryImageId)
		log.Infof("plops memorySignalId= %+v", memorySignalId)
		log.Infof("plops memorySignalContainerId= %+v", memorySignalContainerId)
		log.Infof("plops memorySignalName= %+v", memorySignalName)
		log.Infof("plops memorySignalArgs= %+v", memorySignalArgs)
		log.Infof("plops memorySignalExecFilePath= %+v", memorySignalExecFilePath)
